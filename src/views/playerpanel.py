...
class PlayerPanel(ft.Container):
    def __init__(self, view):
        ...
        # 1. Definimos y guardamos los controles en variables locales para asegurar la referencia
        self.slider_progreso = ft.Slider(
            min=0, max=100, value=0, 
            active_color="#4A90E2", 
            on_change_end=self.view._seek_music
        )
        self.lbl_tiempo_actual = ft.Text("0:00", size=12)
        self.lbl_tiempo_total = ft.Text("0:00", size=12)
        
        # 2. Pasamos estas referencias a la vista principal para que el hilo las encuentre
        ...
        self.view.btn_play_pause_full = ft.IconButton(
            ...
            icon_size=80, on_click=self.view._toggle_reproduccion
        )
    ...
    def _crear_contenido(self):
        return ft.Column([
            ft.Container(
                # FLECHA A LA IZQUIERDA CORREGIDA
            ...
            ft.Column([self.view.full_txt_titulo, self.view.full_txt_artista], 
                      horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=5, width=320),
            
            ft.Column([
                self.slider_progreso, # Usamos la referencia local
                ft.Row([self.lbl_tiempo_actual, self.lbl_tiempo_total], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
            ], width=320),
